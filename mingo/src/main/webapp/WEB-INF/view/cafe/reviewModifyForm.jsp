	...
	<script>
		$(function(){
			...
			//for (var i=0; i<score_name.length; i++){
				...
				var prev_score = $('#<%=score_name[i]%>_score').val();
				var p_width = prev_score*70+40;
				var p_location = prev_score*70;
				console.log(p_width);
...
<body>
	...
	<br>
	<form action="modifyReview.do" method="post" enctype="multipart/form-data">
		<input type="hidden" name="wifi_score" id="wifi_score" value="${reviewVO.wifi_score }">
		<input type="hidden" name="taste_score" id="taste_score" value="${reviewVO.taste_score }">
		<input type="hidden" name="service_score" id="service_score" value="${reviewVO.service_score }">
		<input type="hidden" name="price_score" id="price_score" value="${reviewVO.price_score }">
		<input type="hidden" name="clean_score" id="clean_score" value="${reviewVO.clean_score }">
		<input type="hidden" name="mood_score" id="mood_score" value="${reviewVO.mood_score }">
		<input type="hidden" name="user_id" value="${reviewVO.user_id }"/>	
		<input type="hidden" name="cafe_id" value="${reviewVO.cafe_id}"/>	  
		
		<div class="my-image">
			 <img id="image_section" src="upload/review/${reviewVO.image }" alt="수정할 리뷰 사진을 등록해주세요"/>
		</div>
		<div>
			<textarea rows="3" cols="30" name="contents" id="review-contents">${reviewVO.contents }</textarea>   
		</div>
		<div>
			<input type="file" name="image_file" id="my-review-image"/>	
		</div>
		<div class="review-btn-div">
			<input type="submit" id="review-btn" value="평가 수정">
		</div>
