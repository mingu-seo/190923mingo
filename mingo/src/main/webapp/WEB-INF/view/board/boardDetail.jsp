		...
		<div class=" board-group shadow ml-4">
			<form action="viewBoard.do" method="post" name="deleteform">
				...
				<div class="mybtn-group">
					...
					<button type="button"
						class="btn float-right mybtn-good <%=likeType == 2 ? "likeBtnActive" : ""%>"
						name="likeBad" id="likeBad" onclick="upLikeNum(2);">
						<i class="fa fa-thumbs-o-down" style="font-size: 1.5em;">${dislikeNum }</i>
					</button>
					<button type="button"
						class="btn float-right mybtn-bad <%=likeType == 1 ? "likeBtnActive" : ""%>"
					...
					<div class="reply flex-grow-1">
						...
						<form id="form_${BoardCommentVO.board_comment_id }"
							...
							<input type="hidden" name="type" value="${data.type }"> <input
								...
								type="hidden" name="s2" value="${s2 }"> <input
								type="hidden" name="k" value="${k }">
								...
								<c:choose>
									...
									<c:otherwise>
										<button type="button" class="btn btn-secondary"
											onclick="replyShow('${BoardCommentVO.board_comment_id }')">댓글</button>
										<c:if
											test="${sessionScope.userVO.user_id == BoardCommentVO.user_id}">
											<button type="button" class="btn btn-secondary"
												onclick="replyDelete(${BoardCommentVO.board_comment_id})">삭제</button>
										</c:if>
									</c:otherwise>
						...
						<div class="reply-form p-3">
							<div style="text-align: center; margin-left: auto;">
								로그인 시 댓글 등록이 가능합니다. <a href="loginForm.do"
									style="text-decoration: underline;">로그인하기</a>
							</div>
						</div>
					</c:when>
					<c:otherwise>
						<div class="reply-form p-3">
							<input type="hidden" name="page" value="<%=vo.getPage()%>" /> <input
								...
								type="hidden" name="s1" value="${s1 }" /> <input type="hidden"
								name="s2" value="${s2 }" /> <input type="hidden" name="k"
								value="${k }" /> <input type="hidden" name="board_id"
							...
							<textarea class="mt-2" rows="4"
								...
								placeholder="주제와 무관한 댓글, 악플은 삭제 될 수 있습니다."></textarea>
							<div>
								<button type="button" class="btn btn-secondary"
